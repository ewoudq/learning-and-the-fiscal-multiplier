function output = NewKeynesianSystemEq(input,shocks,states,beliefs,loc_x_obs,loc_z_obs,phi_mat,names_z_obs,parameters)
...
switch sub
    case 1
        output(7)=phi1*beliefs(4,:)*XX-phi2*beliefs(8,:)*XX...
...
switch sub
    case 1
output(9)=-q-r+beliefs(9,:)*XX+beta*(rk_ss*beliefs(11,:)*XX...
...
switch sub
    case 1
        output(11)=-pi+(1-theta)*(1-beta*theta)*mc/theta+beta*beliefs(9,:)*XX;
    case 2 % expectations about wages, rk, and technology
        output(11)=-pi+(1-theta)*(1-beta*theta)*mc/theta+beta*(1-theta)*(1-beta*theta)*(...
            (1-alpha)*beliefs(13,:)*inv(eye(nobs)-beta*theta*TT)*XX...
            +alpha*beliefs(11,:)*inv(eye(nobs)-beta*theta*TT)*XX...
            -rhoz*z/(1-beta*theta*rhoz))...
            +beta*(1-theta)*beliefs(9,:)*inv(eye(nobs)-beta*theta*TT)*XX;
    case 3 % expectations about marginal costs
        output(11)=-pi+(1-theta)*(1-beta*theta)*mc/theta+beta*(1-theta)*(...
            (1-beta*theta)*beliefs(7,:)*inv(eye(nobs)-beta*theta*TT)*XX...
            +beliefs(9,:)*inv(eye(nobs)-beta*theta*TT)*XX);
end

%output(12)=-tk*tauk*rk_ss*ky+g*gy+by*(r_lagged-pi)/beta-tauk*rk_ss*ky*(k_lagged+rk);
output(12)=-t*ty+g*gy+by*(r_lagged-pi)/beta;
