function Output = DebtAdjustment(parameters,irf_shock,sigmag,sigmaz,sigmar,T,S,draw,itermax,damp,constant,gain)
%DistortionaryTaxes Summary of this function goes here
...
rhob=parameters(26); %transfers debt coefficient
...
%% Variable names
VARNAMES = ['b ';'k ';'r ';'y ';'c ';'d ';'i ';'mc';'n ';'pi';'q ';'rk';'tr';'w ';'g ';'ur';'z ']; 
names_x = {'b','k','r','y'}; 
names_y = {'c','d','i','mc','n','pi','q','rk','tr','w'}; 
names_z = {'g','ur','z'}; 
names_disturbances = {'eg','er','ez'}; 
names_x_obs={'k'};
%names_x_obs={'b','k','r'}; 
names_z_obs={'ur','z'}; 
%names_z_obs={'ur','g','z'}; 
names_endogenous=[names_x names_y];
model=4; %4=distortionary taxation with debt adjustments

OrderAndReorder;

%%
DebtOptimalityConditions;
...
if constant==1
    ...
    if loc_z_obs==0 % if no constant and no observed shocks
        ...
        data_res = NeoClassical.REE_Simul.y(2:end,loc_x_obs);
    else % if no constant and observed shocks
